...
export default function StressSelfAssessmentSection() {
  ...
  const calculateResults = (): AssessmentResult => {
    ...
    const highestDimension = Object.entries(dimensionScores)
      .reduce((max, [dim, score]) => score > max.score ? { dimension: dim, score } : max, 
              { dimension: '', score: -1 }).dimension;
    ...
    switch (level) {
      case 'low':
        feedback = 'Votre niveau de stress semble faible. Vous gérez bien la pression au quotidien.';
        advice = [
        ...
        break;
      case 'moderate':
        feedback = 'Votre niveau de stress est modéré. Il est temps de mettre en place des stratégies préventives.';
        advice = [
        ...
        break;
      case 'high':
        feedback = 'Votre niveau de stress est élevé. Il est important d\'agir rapidement pour votre bien-être.';
        advice = [
    ...
    if (highestDimension === 'Signaux cognitifs') {
      ...
      advice.push('Réorganisez votre environnement pour limiter les interruptions');
...
En cas de détresse importante, consultez un professionnel de la santé.
  ...
  const getLevelIcon = (level: string) => {
    switch (level) {
      ...
      case 'moderate': return <AlertCircle className="w-6 h-6 text-orange-500" />;
      ...
      <section className="py-20 bg-gradient-to-br from-blue-50 to-indigo-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <Card className="mb-8 border-2 border-primary/20">
            <CardHeader className="text-center">
              <div className="flex items-center justify-center space-x-4 mb-4">
                ...
                <h3 className="text-2xl font-bold">
                  Niveau de stress: {
                    results.level === 'low' ? 'Faible' :
                    results.level === 'moderate' ? 'Modéré' : 'Élevé'
                  }
                </h3>
              </div>
            ...
            <CardContent>
              <div className={`p-4 rounded-lg border ${getLevelColor(results.level)} mb-6`}>
                <p className="text-center font-medium">{results.feedback}</p>
              </div>

              {/* Dimension Scores */}
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                <h4 className="md:col-span-2 text-lg font-semibold mb-4">Scores par dimension :</h4>
                {Object.entries(results.dimensionScores).map(([dimension, score]) => (
                  <div key={dimension} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <span className="font-medium">{dimension}</span>
                    <span className={`font-bold ${dimension === results.highestDimension ? 'text-red-600' : 'text-gray-700'}`}>
                      {score}/9
                    </span>
                  </div>
                ))}
          ...
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg text-sm text-yellow-800">
            ...
            <p>
              Cet outil est destiné à l'auto-repérage et ne constitue pas un diagnostic médical. 
              En cas de détresse importante ou persistante, n'hésitez pas à consulter un professionnel de la santé.
            </p>
          </div>
        </div>
      </section>
          ...
          <Card className="text-center border-2 border-primary/20">
            <CardContent className="py-12">
              ...
              <p className="text-lg text-gray-600 mb-8">
                Découvrez votre profil de stress personnalisé et nos conseils adaptés.
              </p>
